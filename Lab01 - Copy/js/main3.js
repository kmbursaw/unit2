...
function pointToLayer(feature, latlng){ //attributes removed
    ...
    //var rank = feature.properties.rank; // Rank assigned in the previous step

    //Give each feature's circle marker a radius based on its attribute value
    options.radius = calcPropRadius(attValue);

    //let formattedNumber = numberInMillions.toFixed(2) + "million";
    var formattedNumber = attValue.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",");
    ...
    //popupContent += "<p><b>" + attribute + ":</b> " + formattedNumber + " Passengers</p>";

    //popupContent += "<p><b>Rank:</b> " + rank + "</p>";
